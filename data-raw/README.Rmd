...
set.seed(2017)
MethylatedBS_sim <- apply(N_BS, c(1,2), function(x) sum(rbinom(n=x, size=1, prob=(p_m+p_h))))
                          
UnMethylatedBS_sim <- N_BS - MethylatedBS_sim

MethylatedOxBS_sim <- apply(N_OxBS, c(1,2), function(x) sum(rbinom(n=x, size=1, prob=p_m)))
                            
UnMethylatedOxBS_sim <- N_OxBS - MethylatedOxBS_sim

MethylatedTAB_sim <- apply(N_TAB, c(1,2), function(x) sum(rbinom(n=x, size=1, prob=p_h)))
