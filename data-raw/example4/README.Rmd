...
tmp1 <- apply(MethylatedBS+UnMethylatedBS,1,function(x) min(x,na.rm=TRUE))
tmp2 <- apply(MethylatedOxBS+UnMethylatedOxBS,1,function(x) min(x,na.rm=TRUE))
aa <- which(tmp1>9 & tmp2>9) # only CpGs with at least 10 total read counts in BS and in oxBS across samples
set.seed(2018)
...
rm(MethylatedBS,UnMethylatedBS,UnMethylatedOxBS,MethylatedOxBS,tmp1,tmp2)
...
results_exact <- MLML(T.matrix = Tm, 
        ...
        L.matrix = Lm, 
        M.matrix = Mm)
print(Sys.time()-start) # 3.442421 secs
...
save(hmC_naive,mC_naive,C_naive,file="results_naive.rds")
```
