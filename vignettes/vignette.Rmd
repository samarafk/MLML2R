...
package: MLML2R
abstract: >
  We present a guide to the \R{} package \CRANpkg{MLML2R}. The package provides computationally efficient maximum likelihood estimates of DNA methylation and hydroxymethylation proportions when data from the DNA processing methods bisulfite conversion (BS), oxidative bisulfite conversion (ox-BS), and Tet-assisted bisulfite conversion (TAB) are available. Estimates can be obtained by combining any two of the methods, or all the three methods (if available). The package does not depend on other \R{} packages, allowing the user to read and preprocess the data with any given software, to import the results into \R{} in matrix format, to obtain the maximum likelihood 5-hmC and 5-mC estimates and use them as input for other packages traditionally used in genomic data analysis, such as \Biocpkg{minfi}, \Biocpkg{sva} and \Biocpkg{limma}.
...
--------------------|----------------------------------------------------
\Robject{G.matrix}  | Converted cytosines (T counts or U channel) from TAB-conversion (reflecting 5-C + 5-mC). 
\Robject{H.matrix}  | Unconverted cytosines (C counts or M channel) from TAB-conversion (reflecting True 5-hmC). 
\Robject{L.matrix}	| Converted cytosines (T counts or U channel) from oxBS-conversion (reflecting 5-C + 5-hmC). 
\Robject{M.matrix}	| Unconverted cytosines (C counts or M channel) from  oxBS-conversion (reflecting True 5-mC). 
\Robject{U.matrix}	| Converted cytosines (T counts or U channel) from standard BS-conversion (reflecting True 5-C). 
\Robject{T.matrix}	| Unconverted cytosines (C counts or M channel) from  standard BS-conversion (reflecting 5-mC+5-hmC). 
...
```{r,eval=FALSE}
MChannelBS <- getMeth(MSet.noob)[,c(1,3,5,6)]
UChannelBS <- getUnmeth(MSet.noob)[,c(1,3,5,6)]
MChannelOxBS <- getMeth(MSet.noob)[,c(7,8,2,4)]
UChannelOxBS <- getUnmeth(MSet.noob)[,c(7,8,2,4)]
```
...
```{r,echo=FALSE,fig.width=15,fig.height=10,fig.cap="Estimated proportions of hydroxymethylation, methylation and unmethylation for the CpGs in the dataset from Field (2015) using the MLML function with default (PAVA) options (top row) and the naïve (subtraction) method (bottom row)."}
...
beta_BS <- nbeta[,BSindex]
beta_TAB <- nbeta[,TABindex]
...
MChannelBS <- beta_BS*TotalBS
UChannelBS <- (1-beta_BS)*TotalBS
MChannelTAB <- beta_TAB*TotalTAB
UChannelTAB <- (1-beta_TAB)*TotalTAB
```
...
One needs to carefully check if the columns across the different input matrices represent the same sample. In this example, all matrices have the samples consistently represented in the columns: sample 1 in the first column, sample 2 in the second, and so forth.
 ...
 When any two of the methods are available, the default option of \Rfunction{MLML} function returns the exact constrained maximum likelihood estimates  using the the pool-adjacent-violators algorithm (PAVA) [@ayer1955].
...
```{r,echo=FALSE,fig.width=15,fig.height=10,fig.cap="Estimated proportions of hydroxymethylation, methylation and unmethylation for the CpGs in the dataset from Thienpont et al. (2016), using the MLML function with default (PAVA) options (top row) and the naïve (subtraction) method (bottom row)."}
knitr::include_graphics("Real2a_estimates.pdf") 
```

## Publicly available sequencing data: oxBS and BS methods

We will use the dataset from @Li2016, which consists of three human lung normal-tumor pairs (six samples). Each sample was divided into two replicates: one treated with BS and the other with oxBS, which were then sequenced using the Illumina HiSeq 2000 (Homo sapiens) platform. The dataset is available at GEO accession [GSE70090](https://www.ncbi.nlm.nih.gov/geo/query/acc.cgi?acc=GSE70090). 
...
Preparing the data for input in the \Rfunction{MLML} function:


```{r,echo=TRUE,eval=FALSE}
### BS
files <- phenoLung$file[phenoLung$convMeth=="BS"]
MChannelBS    <- do.call(cbind,lapply(files,function(fn) 
...
UChannelBS <- TotalBS - MChannelBS


### oxBS
files <- phenoLung$file[phenoLung$convMeth=="oxBS"]
MChannelOxBS    <- do.call(cbind,lapply(files,function(fn) 
  fread(fn,data.table=FALSE,select=c("methylated_read_count"))))
TotalOxBS    <- do.call(cbind,lapply(files,function(fn) 
  fread(fn,data.table=FALSE,select=c("total_read_count"))))
UChannelOxBS <- TotalOxBS - MChannelOxBS
...
rownames(MChannelBS) <- CpG 
rownames(UChannelBS) <- CpG

colnames(MChannelBS) <- phenoLung$id[phenoLung$convMeth=="BS"]
colnames(UChannelBS) <- phenoLung$id[phenoLung$convMeth=="BS"]


rownames(MChannelOxBS) <- CpG 
rownames(UChannelOxBS) <- CpG

colnames(MChannelOxBS) <- phenoLung$id[phenoLung$convMeth=="oxBS"]
colnames(UChannelOxBS) <- phenoLung$id[phenoLung$convMeth=="oxBS"]

Tm = as.matrix(MChannelBS)
Um = as.matrix(UChannelBS)
Lm = as.matrix(UChannelOxBS)
Mm = as.matrix(MChannelOxBS)
```
...
tmp1 <- rowMins(TotalBS,na.rm=TRUE) # minimum coverage across samples for each CpG
tmp2 <- rowMins(TotalOxBS,na.rm=TRUE) # minimum coverage across samples for each CpG
...
Comparing the estimates for 5-hmC proportions from iterative and non iterative option from \Rfunction{MLML} function:
```{r,echo=TRUE,eval=FALSE}
all.equal(results_exact$hmC,results_em$hmC,scale=1)
```
...
```{r,echo=FALSE,fig.width=15,fig.height=10,fig.cap="Estimated proportions of hydroxymethylation, methylation and unmethylation for the CpGs in the dataset using the MLML function with default options (top row) and the naïve method (bottom row)."}
