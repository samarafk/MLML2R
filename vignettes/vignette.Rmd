...
set.seed(123)
MSet.swan <- preprocessSWAN(rgSet=rgSet,mSet=MSet.noob)
```


After the preprocessing steps we can use \Rfunction{MLML} from the \CRANpkg{MLML2R} package.


```{r,eval=FALSE}
MChannelBS <- getMeth(MSet.swan)[,BSindex]
UChannelBS <- getUnmeth(MSet.swan)[,BSindex]
MChannelOxBS <- getMeth(MSet.swan)[,oxBSindex]
UChannelOxBS <- getUnmeth(MSet.swan)[,oxBSindex]
```
...
plot(results_exact$hmC,results_em$hmC)
```
...
The input in the \Rfunction{MLML} function accepts as input a \Rclass{MethylSet}, which contains the methylated and unmethylated signals. We used the function \Rfunction{preprocessNoob} [@noob] for background correction, dye bias normalization and construction of the \Rclass{MethylSet}. We then proceeded with the function \Rfunction{preprocessSwan} [@Maksimovic2012] for probe-type normalization.
...
MChannelBS <- getMeth(MSet.swan)[,BSindex]
UChannelBS <- getUnmeth(MSet.swan)[,BSindex]
MChannelTAB <- getMeth(MSet.swan)[,TABindex]
UChannelTAB <- getUnmeth(MSet.swan)[,TABindex]
```
...
```{r,echo=FALSE,fig.width=15,fig.height=10,fig.cap="Estimated proportions of hydroxymethylation, methylation and unmethylation for the CpGs in the dataset from Li et al (2016) using the MLML function with default options (top row) and the naïve method (bottom row)."}
